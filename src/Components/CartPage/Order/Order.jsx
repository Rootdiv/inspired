...
export const Order = ({ cartItems }) => {
  ...
  const validationSchema = Yup.object({
    ...
    phone: Yup.string()
      ...
      .matches(/^\+\d{1}\(\d{3}\)-\d{3}-\d{2}-\d{2}$/, 'Не корректный номер телефона'),
    email: Yup.string().email('Некорректный формат email').required('Заполните email'),
    delivery: Yup.string().required('Выберете способ доставки'),
  });
          ...
          <Form className={style.form}>
            <fieldset className={style.personal}>
              <label className={style.label}>
                <Field className={style.input} type="text" placeholder="ФИО*" name="fio" />
                <ErrorMessage className={style.error} name="fio" component={'span'} />
              </label>
              <label className={style.label}>
                <Field className={style.input} type="text" placeholder="Адрес доставки" name="address" />
              ...
              <label className={style.label}>
                <Field className={style.input} type="email" placeholder="E-mail*" name="email" />
                <ErrorMessage className={style.error} name="email" component={'span'} />
              </label>
            </fieldset>
            <fieldset className={style.radioList}>
              <label className={style.radio}>
                <Field className={style.radioInput} type="radio" name="delivery" value="delivery" />
                <span>Доставка</span>
              </label>
              <label className={style.radio}>
                <Field className={style.radioInput} type="radio" name="delivery" value="self" />
                <span>Самовывоз</span>
              </label>
